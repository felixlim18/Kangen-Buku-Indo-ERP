...
# 2. Add validation to handleSavePurchaseOrder
validation_str = """    if (!supplierOrderNumber.trim()) {
      triggerShake('supplierOrderNumber');
      hasError = true;
    }"""
new_validation_str = validation_str + """
    if (!supplierTrackingNumber.trim()) {
      triggerShake('supplierTrackingNumber');
      hasError = true;
    }"""
content = content.replace(validation_str, new_validation_str)

# 3. Add to payload
payload_str = "supplierOrderNumber: supplierOrderNumber.trim(),"
...
# 5. JSX changes
jsx_old = """              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-6 border-b border-[#E7E1D3]/50 dark:border-neutral-800 font-text">
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    disabled={isPoViewOnly}
                    placeholder="Contoh: ID-9080-X atau Tracking ID"
...
jsx_new = """              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-6 border-b border-[#E7E1D3]/50 dark:border-neutral-800 font-text">
...
if jsx_old in content:
    content = content.replace(jsx_old, jsx_new)
else:
    print("WARNING: JSX OLD not found!")
